...
applyMod('Mod 4: Archive button',
  ...
  `// Store report data for archiving
    ...
    let html = \`<div style="font-size:12px;color:var(--gray-600);margin-bottom:12px">
      <strong>Period:</strong> \${data.payPeriod.label} · <strong>Generated:</strong> \${new Date().toLocaleString()}
      <span style="margin-left:12px;color:var(--gold);font-weight:600">📄 Click "Download PDF" above to generate the full report with W-4s and auto-save to archives.</span>
    </div>\`;`
);
...
async function renderPayrollArchives() {
  ...
  let html = \`<div class="card"><div class="card-header"><h3>Payroll Report Archives (\${archives.length})</h3>
    <div style="font-size:12px;color:var(--gray-400)">PDF reports auto-saved when downloaded</div></div><div class="card-body">\`;

  if (archives.length === 0) {
    html += \`<div class="empty-state"><div class="icon">🗂️</div><p>No archived reports yet. Click "Download PDF" on the Payroll Processing page to generate and auto-archive a report.</p></div>\`;
  } else {
    html += \`<table class="data-table"><thead><tr><th>Pay Period</th><th>Pay Date</th><th>Generated By</th><th>Date</th><th>Actions</th></tr></thead><tbody>\`;
    archives.forEach(a => {
      ...
      html += \`<tr>
        ...
        <td style="white-space:nowrap">
          \${a.has_pdf ? \`<a href="/api/payroll-archives/\${a.id}/pdf" class="btn btn-gold btn-sm" target="_blank">📄 Download PDF</a> \` : ''}
          <button class="btn btn-outline btn-sm" onclick="viewArchivedReport(\${a.id})">View Data</button>
