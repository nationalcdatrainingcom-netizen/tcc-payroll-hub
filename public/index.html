...
@media (max-width: 768px) {
  .sidebar { width: 60px; }
  .sidebar .nav-item span, .sidebar .nav-section, .sidebar-header h2, .sidebar-header .role-badge, .sidebar-header .center-name { display: none; }
...
function buildNav() {
  ...
  let items = [
    { id: 'dashboard', icon: '📊', label: 'PTO Summary', roles: ['owner','payroll','hr','director'] },
    { id: 'employees', icon: '👥', label: 'Employee Roster', roles: ['owner','payroll','hr','director'] },
  ];
  
  if (role === 'director') {
    items.push({ section: 'Director Payroll Processing' });
    items.push({ id: 'director-payroll', icon: '📋', label: 'Process Payroll', roles: ['director'], showDue: true });
    items.push({ id: 'timeoff-calendar', icon: '🗓️', label: 'Monthly View Time Off', roles: ['director'] });
    items.push({ id: 'timeoff-yearly', icon: '📊', label: 'Yearly View Time Off', roles: ['director'] });
  } else {
    items.push({ id: 'timeoff', icon: '📅', label: 'Enter Time Off', roles: ['owner','payroll','hr'] });
    items.push({ id: 'timeoff-calendar', icon: '🗓️', label: 'Monthly View Time Off', roles: ['owner','payroll','hr'] });
    items.push({ id: 'timeoff-yearly', icon: '📊', label: 'Yearly View Time Off', roles: ['owner','payroll','hr'] });
  }
  
  if (['owner','payroll','hr'].includes(role)) {
    items.push({ section: 'Payroll' });
    items.push({ id: 'payroll', icon: '💰', label: 'Payroll Processing', roles: ['owner','payroll'] });
    items.push({ id: 'pay-increases', icon: '📈', label: 'Pay Increases', roles: ['owner','payroll','hr'] });
    items.push({ id: 'documents', icon: '📎', label: 'Documents', roles: ['owner','payroll','hr'] });
  }
  
  items.push({ section: 'Compliance' });
  items.push({ id: 'staffing-plan', icon: '📋', label: 'Staffing Plan', roles: ['owner','payroll','hr','director'] });
  
  if (['owner','payroll','hr'].includes(role)) {
    items.push({ id: 'archive', icon: '📦', label: 'Archive', roles: ['owner','payroll','hr'] });
  }
...
function getNextReportDue() {
  if (!payPeriod || !payPeriod.end) return '...';
  const endDate = new Date(payPeriod.end + 'T12:00:00');
...
function navigate(page) {
  ...
  const titles = {
    ...
    'timeoff-calendar': 'Monthly View Time Off',
...
async function renderDashboard() {
  ...
  if (!isDirector) {
    ...
    CENTERS.forEach(c => {
      ...
      const toDone = !!pp.timeoff_approved;
      const dirDone = !!pp.director_closed;
...
async function loadPTOSummary() {
  ...
  try {
    ...
    let html = `<div style="font-size:11px;color:var(--gray-600);background:var(--gold-pale);padding:10px 14px;border-radius:6px;margin-bottom:12px;line-height:1.6">
        ...
        Hours worked updates each time a Playground timecard CSV is uploaded.
      </div>
      <div style="font-size:12px;color:var(--gray-600);margin-bottom:8px"><strong>${details.length}</strong> employees</div>
      <table class="data-table"><thead><tr>
      <th>Name</th><th>Center</th><th>Yrs</th><th>Hrs Worked</th><th>PTO Accrued<br><small>(hrs÷20)</small></th><th>Tenure Days<br><small>(policy chart)</small></th><th>Total Avail</th><th>Used</th><th>Remaining</th><th>Unpaid (6mo)</th>
    </tr></thead><tbody>`;
    ...
    if (dashboardCenter === 'all' && currentUser.role !== 'director') {
      ...
      details.forEach(e => {
        if (e.center !== currentCtr) {
          currentCtr = e.center;
          html += `<tr style="background:var(--navy);color:white"><td colspan="10" style="padding:6px 14px;font-weight:700">${currentCtr}</td></tr>`;
        }
        html += buildPTORow(e);
      });
    } else {
...
function buildPTORow(e) {
  ...
  return `<tr class="clickable" onclick="showEmployeeDetail(${e.id})">
    ...
    <td>${unpaidWarn ? `<span class="badge badge-danger">${p.unpaidLast6Months} ⚠️</span>` : (p.unpaidLast6Months||0)}</td>
  </tr>`;
}
...
function showAddEmployee() {
  let html = `<div class="modal-header"><h3>Add Employee</h3><button class="modal-close" onclick="closeModal()">&times;</button></div>
    <div class="modal-body">
      <div class="form-row">
        <div class="form-group"><label>First Name</label><input id="empFirst" placeholder="First name"></div>
        <div class="form-group"><label>Last Name</label><input id="empLast" placeholder="Last name"></div>
      </div>
      <div class="form-row-3">
        <div class="form-group"><label>Center</label><select id="empCenter">
      ...
      <div class="form-row-3">
        <div class="form-group"><label>Start Date</label><input id="empStartDate" type="date"></div>
      ...
      <div class="form-row">
        ...
        ${canSeePayRate() ? `<div class="form-group"><label>Hourly Rate</label><input id="empRate" type="number" step="0.01" placeholder="0.00"></div>` : ''}
      </div>
...
async function renderTimeOff() {
  ...
  if (isDirector && !isSubmitted) {
    html += `<div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--gray-200);text-align:right">
      <button class="btn btn-primary" onclick="submitTimeOff()" style="font-size:14px;padding:10px 24px">✅ Submit Time Off Report</button>
      <div style="font-size:11px;color:var(--gray-400);margin-top:6px">Once submitted, Jared can begin processing payroll. You can unsubmit if payroll processing hasn't started yet.</div>
    </div>`;
  }
...
async function submitTimeOff() {
  ...
  if (!confirm('Submit the time off report for ' + center + ' for the period ' + payPeriod.label + '?\n\nThis tells Jared the data is ready for payroll processing.')) return;
  try {
...
async function unsubmitTimeOff() {
  ...
  try {
    const res = await fetch('/api/payroll-workflow/unsubmit-timeoff', {
      ...
      body: JSON.stringify({ period_start: payPeriod.start, period_end: payPeriod.end, center })
    });
    const data = await res.json();
    if (!res.ok) {
...
async function requestTimeOffChange() {
  ...
  try {
    await api('/api/payroll-workflow/request-timeoff-change', { method: 'POST', body: {
      period_start: payPeriod.start, period_end: payPeriod.end, center, reason
    }});
...
async function renderTimeOffCalendar() {
  ...
  const filtered = currentUser.role === 'director' ? employees.filter(e => e.center === currentUser.center) : employees;
...
async function renderDirectorPayroll() {
  ...
  let html = `<div style="margin-bottom:16px;font-size:13px;color:var(--gray-600)">
    <strong>Pay Period:</strong> ${payPeriod.label} · <strong>Next report due:</strong> ${getNextReportDue()}
  </div>`;
  
  // Status overview
  html += `<div class="stats-row" style="margin-bottom:20px">
    <div class="stat-card ${tcUploaded ? 'green' : 'red'}">
      <div class="stat-label">Step 1: Timecards</div>
      <div class="stat-value" style="font-size:16px">${tcUploaded ? '✅ Uploaded' : '⏳ Pending'}</div>
    </div>
    <div class="stat-card ${isSubmitted ? 'green' : 'red'}">
      <div class="stat-label">Step 2: Time Off</div>
      <div class="stat-value" style="font-size:16px">${isSubmitted ? '✅ Submitted' : '⏳ Pending'}</div>
    </div>
  </div>`;
  
  // ---- SECTION 1: Upload Timecards ----
  html += `<div class="card" style="margin-bottom:20px"><div class="card-header"><h3>Step 1: Upload Daily Staff Attendance</h3></div>
    <div class="card-body">
      <p style="font-size:13px;color:var(--gray-600);margin-bottom:12px">Upload the Playground "Daily Staff Hours" CSV for <strong>${center}</strong> for the dates <strong>${payPeriod.label}</strong>.</p>
      ${tcUploaded ? '<div style="background:var(--success-bg);border:1px solid var(--success);border-radius:8px;padding:10px 14px;margin-bottom:12px;color:var(--success);font-weight:600">✅ Timecards uploaded for this period</div>' : ''}
      <input type="file" id="dirTimecardFile" accept=".csv" style="margin-bottom:12px">
      <button class="btn btn-primary" onclick="dirUploadTimecard()">Upload CSV</button>
  ...
  if (isSubmitted && !changePending) {
    html += `<div style="background:var(--success-bg);border:1px solid var(--success);border-radius:8px;padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between">
      <div><span style="font-weight:700;color:var(--success)">✅ Time Off Report Submitted</span>
  ...
  html += `<div class="card"><div class="card-header"><h3>Step 2: Enter Time Off — ${payPeriod.label}</h3>
    <div style="font-size:12px;color:var(--gray-400)">${isEditable ? 'Click a cell: Empty → P → U → Clear' : 'Read-only — submitted'}</div>
  </div><div class="card-body"><div class="table-scroll">
    <table class="calendar-grid"><thead><tr><th class="name-col">Employee</th>`;
  ...
  if (!isSubmitted) {
    html += `<div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--gray-200);text-align:right">
      <button class="btn btn-primary" onclick="submitTimeOff()" style="font-size:14px;padding:10px 24px">✅ Submit Time Off Report</button>
      <div style="font-size:11px;color:var(--gray-400);margin-top:6px">Once submitted, Jared can process payroll.</div>
    </div>`;
  }
...
async function dirUploadTimecard() {
  ...
  try {
    const res = await fetch('/api/import-timecards', { method: 'POST', body: formData });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error);
    resultDiv.innerHTML = `<div style="color:var(--success);font-weight:600">✅ Imported ${data.imported} rows, matched ${data.matched} employees, saved ${data.savedDays} daily records.</div>
      ${data.unmatchedNames?.length ? '<div style="color:var(--warning);margin-top:6px">Unmatched: ' + data.unmatchedNames.join(', ') + '</div>' : ''}`;
...
async function renderPayroll() {
  ...
  html += `<div class="card"><div class="card-header"><h3>Payroll Workflow — ${center}</h3></div><div class="card-body">
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px">
      ...
      <div style="padding:14px;border-radius:8px;text-align:center;background:${pp.timeoff_approved?'var(--success-bg)':'var(--gray-100)'}">
        <div style="font-size:20px">${pp.timeoff_approved?'✅':'2️⃣'}</div>
        <div style="font-size:11px;font-weight:700;margin-top:4px">Time Off</div>
        ${pp.timeoff_signed_by ? `<div style="font-size:10px;color:var(--gray-600);margin-top:2px">Signed: ${pp.timeoff_signed_by}</div>` : ''}
      </div>
      <div style="padding:14px;border-radius:8px;text-align:center;background:${pp.director_closed?'var(--success-bg)':'var(--gray-100)'}">
        <div style="font-size:20px">${pp.director_closed?'✅':'3️⃣'}</div>
        <div style="font-size:11px;font-weight:700;margin-top:4px">Director Close</div>
        ${pp.director_closed_by ? `<div style="font-size:10px;color:var(--gray-600);margin-top:2px">${pp.director_closed_by}</div>` : ''}
      </div>
      <div style="padding:14px;border-radius:8px;text-align:center;background:${pp.payroll_closed?'var(--success-bg)':'var(--gray-100)'}">
        <div style="font-size:20px">${pp.payroll_closed?'✅':'4️⃣'}</div>
  ...
  // ---- STEP 3: DIRECTOR CLOSE OUT ----
  if (isDirector) {
    html += `<div class="card"><div class="card-header"><h3>Step 3: Close Out Pay Period</h3>
      ${pp.director_closed ? '<span class="badge badge-success">Submitted</span>' : ''}
    </div><div class="card-body">`;
    
    if (pp.timecards_uploaded && pp.timeoff_approved && !pp.director_closed) {
      html += `<div style="padding:16px;border:2px solid var(--gold);border-radius:8px;background:rgba(200,150,62,0.05)">
        <h4 style="font-size:14px;color:var(--navy);margin-bottom:8px">Final Certification</h4>
        <p style="font-size:12px;color:var(--gray-600);margin-bottom:12px">I certify that all payroll information for this pay period has been reviewed, verified, and is ready for processing.</p>
        <div class="form-group"><label>Type your full name to sign</label><input id="closeSignName" placeholder="Full name"></div>
        <button class="btn btn-primary" onclick="directorClose()">Close Out & Submit to Payroll</button>
      </div>`;
    } else if (pp.director_closed) {
      html += `<div style="background:var(--success-bg);padding:12px;border-radius:8px;font-size:13px;color:var(--success)">
        ✅ Submitted to payroll by <strong>${pp.director_closed_by}</strong> on ${pp.director_closed_at ? new Date(pp.director_closed_at).toLocaleString() : ''}
      </div>`;
    } else {
      html += `<p style="font-size:13px;color:var(--gray-400)">Complete Steps 1 and 2 before closing out.</p>`;
    }
    html += `</div></div>`;
  }
  ...
  if (isPayroll) {
    html += `<div class="card"><div class="card-header"><h3>Payroll Report — ${center}</h3>
      <button class="btn btn-primary btn-sm" onclick="loadPayrollReport()">Generate Report</button>
    </div><div class="card-body"><div id="payrollReportArea"><em style="color:var(--gray-400)">Click "Generate Report" to load.</em></div></div></div>`;
    
    // Close out button for Jared
    if (!pp.payroll_closed) {
      const allClosed = ['Peace Boulevard','Niles','Montessori'].every(c => periods[c]?.director_closed);
      html += `<div class="card"><div class="card-header"><h3>Step 4: Process Payroll</h3></div><div class="card-body">`;
      if (allClosed) {
        html += `<div style="padding:16px;border:2px solid var(--gold);border-radius:8px;background:rgba(200,150,62,0.05)">
          <p style="font-size:13px;color:var(--gray-600);margin-bottom:12px">All centers have submitted. Process payroll and close this period.</p>
        ...
        const waiting = ['Peace Boulevard','Niles','Montessori'].filter(c => !periods[c]?.director_closed);
        html += `<p style="font-size:13px;color:var(--warning)">⏳ Waiting for director submissions: ${waiting.join(', ')}</p>`;
      }
      html += `</div></div>`;
    }
  }
...
async function importTimecard() {
  ...
  if (result.error) {
    ...
    let html = `<div class="confidentiality-banner" style="background:var(--success-bg);border-color:var(--success);color:var(--success)">
    ...
    if (result.unmatched > 0) {
      html += `<div class="confidentiality-banner" style="background:var(--warning-bg);border-color:var(--warning);color:var(--warning)">
        ⚠️ ${result.unmatched} rows could not be matched: ${result.unmatchedNames.join(', ')}
        <br><small>These staff may need to be added to the Employee Roster first.</small>
      </div>`;
    }
...
async function loadPayrollReport() {
  ...
  area.innerHTML = '<em style="color:var(--gray-400)">Generating report...</em>';
  
  try {
    const center = selectedCenter === 'all' ? '' : selectedCenter;
    const data = await api(`/api/payroll-report?date=${payPeriod.start}&center=${center}`);
    ...
    for (const [ctr, emps] of Object.entries(byCenter)) {
      ...
      html += `<div class="table-scroll"><table class="data-table"><thead><tr>
        <th>Name</th><th>Position</th><th>Rate</th><th>Reg Hrs</th><th>OT Hrs</th><th>Total Hrs</th><th>PTO Days</th><th>Unpaid</th><th>Pay Increases</th>
      </tr></thead><tbody>`;
      ...
      emps.forEach(e => {
        ...
        html += `<tr>
          ...
          <td style="font-family:'JetBrains Mono',monospace;font-size:12px">${e.hourly_rate ? '$'+parseFloat(e.hourly_rate).toFixed(2) : '—'}</td>
          ...
          <td>${e.ptoDays > 0 ? '<span class="badge badge-info">'+e.ptoDays+'</span>' : '—'}</td>
          <td>${e.unpaidDays > 0 ? '<span class="badge badge-danger">'+e.unpaidDays+'</span>' : '—'}</td>
...
async function renderStaffingPlan() {
  ...
  const orderedClassrooms = template.map(t => t.section);
  Object.keys(classroomMap).forEach(c => {
    if (!orderedClassrooms.includes(c)) orderedClassrooms.push(c);
  });
  ...
  if (canEdit && unassigned.length > 0) {
    html += `<div class="card" style="border-left:4px solid var(--warning);margin-bottom:20px">
      <div class="card-header"><h3>⚠️ Needs Room/Role Assignment (${unassigned.length})</h3></div>
      <div class="card-body">
  ...
  orderedClassrooms.forEach(classroom => {
    ...
    html += `<tr style="background:var(--navy);color:white">
      <td colspan="${colSpan}" style="padding:8px 14px;font-weight:700">${classroom}</td>
    </tr>`;
    ...
    staff.forEach(s => {
      html += `<tr data-sp-id="${s.id}">
        ${canEdit ? `<td style="text-align:center;cursor:pointer" title="Move to different classroom" onclick="moveStaffEntry(${s.id},'${s.first_name} ${s.last_name}')"><span style="font-size:14px;cursor:grab">☰</span></td>` : ''}
        <td></td>
        <td><strong>${s.role_in_room || ''}</strong></td>
        <td style="white-space:nowrap">${s.first_name || ''} ${s.last_name || ''}</td>
        <td>${fmtDate(s.emp_start_date)}</td>
        <td style="font-size:10px">${s.emp_schedule || ''}</td>
        ${cc(s.orientation_date, true)}
        ${cc(s.cpr_first_aid_date, true)}
        ${cc(s.health_safety_abc_date, true)}
        ${cc(s.health_safety_refresher, true)}
        ${cc(s.ccbc_consent_date, true)}
        ${cc(s.fingerprinting_date, true)}
        ${cc(s.date_eligible, true)}
        ${cc(s.abuse_neglect_statement, true)}
        ${cc(s.last_evaluation, false)}
        ${cc(s.date_promoted_lead, false)}
        ${cc(s.date_assigned_room, false)}
        <td style="font-size:10px">${s.education || ''}</td>
        <td style="font-size:10px">${s.semester_hours || ''}</td>
        <td style="font-size:10px">${s.infant_toddler_training || ''}</td>
...
async function removeStaffingEntry(spId) {
  if (!confirm('Remove this staff member from the staffing plan? Their employee data will be preserved.')) return;
  try {
